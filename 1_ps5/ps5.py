...
def img_to_pix(filename):
    ...
    return Image.open(filename).getdata()
...
def filter(pixels_list, color):
    ...
    for x in pixels_list:
        temp = matrix_multiply(make_matrix(color), x)
        result.append((int(temp[0]), int(temp[1]), int(temp[2])))
    return result
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if type(pixel) is tuple:
        result = (pixel[0]%(2**num_end_bits), pixel[1]%(2**num_end_bits), pixel[2]%(2**num_end_bits))
    else:
        result = pixel%(2**num_end_bits)
    return result
...
def reveal_bw_image(filename):
    ...
    img = img_to_pix(filename)
    result = []
    for x in img:
        result.append((extract_end_bits(1, int(x)))*255)
    return pix_to_img(result, im.size, "L")
...
def reveal_color_image(filename):
    ...
    for x in img:
        temp_result = []
        temp_result.append(extract_end_bits(3, int(x[0]))*36.4286)
        temp_result.append(extract_end_bits(3, int(x[1]))*36.4286)
        temp_result.append(extract_end_bits(3, int(x[2]))*36.4286)
        result.append((int(temp_result[0]), int(temp_result[1]), int(temp_result[2])))
    return pix_to_img(result, im.size, "RGB")
